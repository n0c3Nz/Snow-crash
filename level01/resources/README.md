...
Por lo que llevo este par ( $\color{Cyan}{\textsf{flag01:42hDRfypTqqnw}}$ ) a mi sistema local donde trato de romperlo con john:
```bash
john --format=descrypt hash.txt 
```
> John aplicará un diccionario interno y tratará de generar contraseñas basadas en el nombre del usuario por defecto, las encriptará y comparará los resultados con el hash a romper.
